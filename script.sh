...
read username;
if [ $username = "Alireza" ]
then echo "The Username as $username is Correct.";
else echo "Get the Fuck off.";
fi;;

4)
echo "Enter Your Password:";
read password;
while [ $password != "alireza" ]
do 
...
read number;
if [ $number -lt 0 ]
then echo "The Number is Negetive.";
elif [ $number -gt 0 ]
then echo "The Number is Positive.";
else echo "The Number is Zero(0).";
fi;;
...
read name;
if [ $option = file ]
then touch /home/alireza/Desktop/$name;
else mkdir /home/alireza/Desktop/$name;
fi;;
...
read option;
if [ $option = "l" ]
then cat $fileName | tr "[:upper:]" "[:lower:]";
elif [ $option = "u" ]
then cat $fileName | tr "[:lower:]" "[:upper:]";
else echo "Idiot."
fi;;
...
echo "Result is:";
if [ $option = "y" ]
then cat $fileName | sort | uniq;
else cat $fileName | sort
fi;;
